...
> Укороченные списки VPN-серверов для обхода блокировок. 26 укороченных списков вместо тонны нод. Скопировал ссылку — вставил в клиент — и всё работает без тормозов и вылетов.
>
> Автоматически обновляемая коллекция публичных VPN-конфигов (V2Ray / VLESS / Hysteria / Trojan / VMess / Reality / Shadowsocks) для быстрого обхода блокировок. Каждый конфиг — это TXT-подписка, которую можно импортировать практически в любой современный клиент (v2rayNG, NekoRay, Throne, v2rayN, V2Box, v2RayTun, Hiddify и др.). Списки обновляются раз в минуту с помощью GitHub Actions, поэтому ссылки из раздела «📋 Ссылки на подписки (1–26)» всегда актуальны.
...
В данном репозитории для каждого из 26 списков проекта **[AvenCores/goida-vpn-configs](https://github.com/AvenCores/goida-vpn-configs)** подготовлен **отдельный укороченный файл**: в него входит примерно четверть серверов (отбирается каждая 4‑я строка с конфигом). Уменьшение числа конфигов снижает нагрузку на клиент и обеспечивает стабильную работу без задержек. Обновление списков выполняется **автоматически** — проверка исходных данных выполняется раз в минуту, поэтому изменения появляются вскоре после обновления исходного репозитория.
...
configs/
├── 1.txt … 26.txt           # укороченные подписки (примерно 1/4 серверов)
...
<a id="ссылки-на-подписки-1-26"></a>
## 📋 Ссылки на подписки (1–26)

В каждом файле содержится примерно **четверть конфигов** из соответствующего списка репозитория [AvenCores/goida-vpn-configs](https://github.com/AvenCores/goida-vpn-configs). Учитываются только строки с конфигурациями протоколов **vless**, **trojan**, **vmess**, **ss**, **hy2**; из них отбирается каждая 4‑я строка. Для **обхода блокировок по SNI/CIDR** (белые списки) предназначен список **№26**. Списки №1–№25 соответствуют разным источникам и наборам серверов.

**Рекомендации автора AvenCores/goida-vpn-configs:** в указанном репозитории рекомендуются списки **1, 6, 22, 23, 24, 25**. Список **№26** автор выделяет отдельно — он предназначен для обхода блокировок по SNI/CIDR (белые списки). Подробное описание того, для чего предназначен каждый список и откуда берутся источники, приведено в [репозитории AvenCores/goida-vpn-configs](https://github.com/AvenCores/goida-vpn-configs).
...
При необходимости использовать **полный** список серверов (без сокращения) ссылки можно взять из репозитория **[AvenCores/goida-vpn-configs](https://github.com/AvenCores/goida-vpn-configs)**. Следует учитывать, что большой объём конфигов может приводить к замедлению работы клиента.
...
В репозитории AvenCores/goida-vpn-configs рекомендуются списки 1, 6, 22, 23, 24, 25. Список 26 предназначен для обхода блокировок по SNI/CIDR (белые списки).
...
Обновление укороченных списков выполняется **автоматически**. Workflow GitHub Actions обращается к репозиторию AvenCores/goida-vpn-configs **раз в минуту**; как только там обновляются исходные конфиги, в течение минуты изменения попадают и в файлы данного репозитория. Таким образом, локальные списки в папке `configs/` актуализируются сразу после обновления исходного источника.

Технически: для каждого из 26 списков загружается соответствующий файл, из него отбираются строки с конфигурациями протоколов vless, trojan, vmess, ss, hy2; сохраняется каждая 4‑я строка; результат записывается в `configs/1.txt` … `configs/26.txt`. Собственный сервер не требуется, обработка выполняется на инфраструктуре GitHub.
